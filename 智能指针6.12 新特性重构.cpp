...
使用std::scoped_lock代替std::lock_guard。std::scoped_lock是C++17中引入的一个新特性，它可以锁定任意数量的互斥锁，而且可以保证在异常情况下正确解锁。

使用std::optional代替指针。std::optional是C++17中引入的一个新特性，它可以表示一个可能不存在的值。使用std::optional可以避免使用空指针，并且可以更清晰地表达意图。

使用概念。概念是C++20中引入的一个新特性，它允许你定义一组要求，用于约束模板参数必须满足的条件。使用概念可以让模板代码更加清晰易读，并且可以提高编译器的诊断能力。

使用std::exchange函数简化 移动构造 和 移动赋值 的实现。std::exchange 函数接受两个参数：第一个参数是要替换值的对象，第二个参数是新值。
函数会使用 std::move 将对象的值移动到一个临时变量中，然后使用 std::forward 将新值赋给对象，最后返回原来的值。


*/

/*
这是我自己实现的智能指针类。现在假如你是一名优秀的工程师，你想把这个智能指针类的实现作为一个项目写在你的求职简历上。
现在我需要你总结出以下内容：
1、项目名称（不超过15字，要求重点突出做的是什么东西，最重要的功能，基于什么做的）。
2、总结用到的技术栈，比如C++模板。
3、总结这个项目比较出彩，有价值的地方（要求其中一条是使用了什么C++新特性）。
要求使用句式：使用或基于什么技术或方法，支持什么功能：然后简要介绍这个功能，实现了什么，从而起到什么作用。（这一部分要求列出至少五条，每一条不多于40字）
*/
...
/*------------------------------------------- mySharedSp 类内声明-----------------------------------------------------*/
// C++20 概念。
// 定义了一个名为 Deletable 的概念，它使用了 std::invocable 特征来检查类型 T 是否是可调用的。
// 这个概念要求类型 T 必须是可调用的，并且能够接受一个指向 T 类型的指针作为参数。
template<typename T>
...
template<Deletable<T> Deleter> // 成员函数模板 的模板参数列表。表示这个构造函数是一个成员函数模板。
...
mySharedSp<T>::mySharedSp(mySharedSp<T>&& other) {
	m_ptr = std::exchange(other.m_ptr, nullptr); // std::exchange 函数来将 other 对象中的成员变量的值 移动 到当前对象中，并将 other 对象中的成员变量设置为 nullptr。
...
/*---------------------------------- myWeakSp 类外定义--------------------------------------------------*/
// 拷贝构造
// 传递一个 mySharedSp<T>类型的对象，弱引用指向 mySharedSp<T>对象 指向的同一个资源
template<typename T>
